# PSOC&trade; Control C3M/P8: TCPWM in PWM mode with dual compare/capture

This code example demonstrates how to generate asymmetric PWM signals using two compare/capture registers available in the TCPWM block of Infineon's PSOC&trade; Control C3M/P8 MCUs. Compared to the asymmetric PWM realized with only one compare function (where the CPU is used to update the compare value twice every PWM cycle), this solution uses two independent buffered compare values and causes less load on the CPU (where the CPU is used to update the compare value once every PWM cycle). This code example discusses these advantages and applications in field-oriented control (FOC).


[View this README on GitHub.](https://github.com/Infineon/mtb-example-ce241644-pwm-dual-compare-capture)

[Provide feedback on this code example.](https://yourvoice.infineon.com/jfe/form/SV_1NTns53sK2yiljn?Q_EED=)
...
The following example clones the "[mtb-example-ce241644-pwm-dual-compare-capture](https://github.com/Infineon/mtb-example-ce241644-pwm-dual-compare-capture)" application with the desired name "PwmDualCompare" configured for the *KIT_PSC3M8_EVK* BSP into the specified working directory, *C:/mtb_projects*:

   ```
   project-creator-cli --board-id KIT_PSC3M8_EVK --app-id mtb-example-ce241644-pwm-dual-compare-capture --user-app-name PwmDualCompare --target-dir "C:/mtb_projects"
   ```
...
4. After programming, the application starts automatically. Confirm that "PSOC Control C3M/P8: TCPWM in PWM mode with dual compare/capture" is displayed and instructions are printed on the terminal
...
5. Tap out the following pins to an oscilloscope:
   ...
   KIT-PSC3M8-EVK:
   1. P8_0 - PWM output (periodic square wave)
   2. P8_2 - Overflow (marks the center of the period)

6. On the oscilloscope, verify that a center-aligned square wave with a 50% duty cycle is generated on P8_0. Use the signal on P8_2 to verify the alignment
...
This code example demonstrates how to generate asymmetric PWM signals using dual compare/capture. The configuration required for the TCPWM block is done in a custom *design.modus* file.
...
The advantage of using dual compare/capture registers to generate asymmetric PWM signals is the reduction in CPU bandwidth usage. With only one CC register, the application must write new values to CC_Buff registers every half a cycle. With two CC registers, the application needs to write new values to CC_Buff registers only once every cycle, thereby reducing the CPU bandwidth usage by half.

Asymmetric PWMs are widely used in FOC to drive the gates of MOSFET bridges. The duty cycle of the PWM signals is modulated in the form of a sine wave to generate the required vectors. Asymmetric PWMs are used to introduce temporary phase shifts to measure the single-shunt current. The single-shunt design reduces the cost and complexity of the motor control application significantly.
...
**Table 1. Application resources**
 ...
 :--------       | :-------------     | :------------
 TCPWM (PDL)     | PWM1 (line)        | PWM block to generate asymmetric waveforms
 UART (HAL)      | cy_retarget_io_uart_obj | UART HAL object used by retarget-io for debug UART port
 GPIO (PDL)      | P8.0 (pwm_output)  | Brings out the PWM output signal
 GPIO (PDL)      | P8.2 (overflow)    | Brings out the overflow signal
...
Document title: *CE241644* – *PSOC&trade; Control C3M/P8: TCPWM in PWM mode with dual compare/capture*
